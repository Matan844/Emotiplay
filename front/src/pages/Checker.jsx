...
export default function Checker() {
  ...
  const { watched, setWatched } = useContext(Storage);
  ...
  function finishingFunc() {
    const videoId = filterdVideo[counter]._id;
  ...
  const saveWatchedVideo = (videoId) => {
    ...
    axios
      .post("http://localhost:8639/user/getallviedvideos", { userId: userId })
      .then((response) => {
        const viewdvideo = response.data.message;
        for (let i = 0; i < viewdvideo.length; i++) {
          if (videoId == viewdvideo[i]) {
            setsame(true);
          }
        }
      });
    if (same == true) {
      console.log("you try save viewd video");
    } else {
      axios
        .post("http://localhost:8639/user/watchedVideoSave", {
          userId: userId,
          videoId: videoId,
        })
  ...
  const review = (body) => {
    axios
      .put(
        `http://localhost:8639/rate/rateVideo/${filterdVideo[counter]?._id}`,
        body
      )
      .then((response) => console.log(response))
      .catch((error) => console.log(error));
  ...
  const inappropriate = () => {
    axios
      .post(`http://localhost:8639/rate/rateVideo/${filterdVideo[counter]?._id}`)
      .then((response) => console.log(response))
      .catch((error) => console.log(error));
  };
  ...
  return (
    <div className="w-full h-full flex flex-row justify-center bg--50 mt-10">
      <div className="w-1/6 flex justify-center items-center">
        <button
          className=" bg-orange-600 rounded p-3 text-white text-xl"
          onClick={() => {
            navigate("/enter");
            localStorage.removeItem("wrongAnswer");
            localStorage.removeItem("firstRandom");
      ...
      {nextPage == true ? (
        <div className="w-1/6 flex justify-center items-center  ">
          <a
            className="object-none  rounded p-3 text-white text-xl bg-blue-600"
        ...
        <div className="w-1/6 flex justify-center items-center  ">
          ...
          <button onClick={() => console.log(filterdVideo)}>
            sssssssss
          </button>
        </div>
      )}
